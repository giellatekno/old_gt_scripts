...
sub copy_file_to_word_ratio_check {
    my ($file_to_copy) = @_;
    (my $to_file = $file_to_copy) =~ s/\/converted\//\/word_ratio_check\/converted\//;
    my $parallel_path = substr($to_file, 0, rindex($to_file, "/"));
    if (! -e $parallel_path) {
        File::Path::mkpath($parallel_path);
    }
            ...
            if ($abs_path_wordcount > 30) {
                ...
                if ($ratio > 73 and $ratio < 110) {
                    ...
                    if ($quiet) {
                        ...
                        #cip: only files complying with specific conditions (Berit Merete and Marja know which ones) should be copied over
