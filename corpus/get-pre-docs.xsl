...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/" name="main">
    <xsl:variable name="pre_files">
      <pre_files>
	
	<xsl:attribute name="counter">
	  <xsl:value-of select="count(document($inFile)/corpus_summary/file[size/ne_pre_count &gt; 0])"/>
	</xsl:attribute>
	
	<xsl:for-each select="document($inFile)/corpus_summary/file[size/ne_pre_count &gt; 0]">
	  <xsl:copy-of select="."/>
	</xsl:for-each>
      </pre_files>
    </xsl:variable>
    
    <xsl:result-document href="{$outDir}/pre_elem_files.{$e}" format="{$outFormat}">
      <xsl:copy-of select="$pre_files"/>
    </xsl:result-document>
